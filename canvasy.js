...
async function createCompositeImage(profileImageUrl, memberCount) {
  try {
    ...
    const resizedWidth = canvas.width * 0.3;
    const resizedHeight = canvas.width * 0.3;
    ...
    const text = `Welcome! You're the ${memberCount}th member!`;
    const textWidth = ctx.measureText(text).width;
    const textXPosition = (canvas.width - textWidth) / 2;

    // Adjust the `y` position to be below the profile image
    const textYPosition = yPosition + resizedHeight + 40; // Adjust as needed for spacing
